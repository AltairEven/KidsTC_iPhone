...
@interface KTCSearchSortFilterView () <UITableViewDataSource, UITableViewDelegate>
...
@implementation KTCSearchSortFilterView
...
- (id)awakeAfterUsingCoder:(NSCoder *)aDecoder
{
    ...
    {
        bLoad = YES;
        KTCSearchSortFilterView *view = [GConfig getObjectFromNibWithView:self];
        return view;
    }
    bLoad = NO;
    return self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(searchSortFilterView:didSelectedAtIndex:)]) {
            [self.delegate searchSortFilterView:self didSelectedAtIndex:indexPath.row];
        }
        if ([self.delegate respondsToSelector:@selector(searchSortFilterViewNeedDismiss:)]) {
            [self.delegate searchSortFilterViewNeedDismiss:self];
        }
    }
}
