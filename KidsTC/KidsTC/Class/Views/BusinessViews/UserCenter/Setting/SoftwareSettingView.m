...
@interface SoftwareSettingView () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate>
...
@property (nonatomic, strong) UITextField *sysNoInputField;

- (void)didClickedSegueButton;
...
#ifdef KIDSTC_DEBUG
    self.sysNoInputField = [[UITextField alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH - 20, 40)];
    [self.sysNoInputField setBackgroundColor:[UIColor yellowColor]];
    self.sysNoInputField.delegate = self;
    [self.sysNoInputField setReturnKeyType:UIReturnKeyGo];
    self.tableView.tableFooterView = self.sysNoInputField;
#endif
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (self.delegate && [self.delegate respondsToSelector:@selector(softwareSettingView:didClickedSegueButtonWithSysNo:)]) {
        [self.delegate softwareSettingView:self didClickedSegueButtonWithSysNo:textField.text];
    }
    return NO;
}
