//
//  OrderDetailButtonCell.h
//  KidsTC
//
//  Created by 钱烨 on 7/27/15.
//  Copyright (c) 2015 KidsTC. All rights reserved.
//

#import <UIKit/UIKit.h>
#import "OrderDetailModel.h"

@class OrderDetailButtonCell;

@protocol OrderDetailButtonCellDelegate <NSObject>

- (void)didClickedPayButtonOnOrderDetailButtonCell:(OrderDetailButtonCell *)cell;

- (void)didClickedCommentButtonOnOrderDetailButtonCell:(OrderDetailButtonCell *)cell;

- (void)didClickedCancelButtonOnOrderDetailButtonCell:(OrderDetailButtonCell *)cell;

- (void)didClickedReturnButtonOnOrderDetailButtonCell:(OrderDetailButtonCell *)cell;

@end

@interface OrderDetailButtonCell : UITableViewCell

@property (nonatomic, assign) id<OrderDetailButtonCellDelegate> delegate;

@property (nonatomic, assign) OrderStatus status;

@property (nonatomic, assign) BOOL supportRefund;

+ (CGFloat)cellHeight;

+ (BOOL)willShowWithOrderStatus:(OrderStatus)status;

@end
