...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, iCoverFlow *self);
...
- (void)setDataSource:(id<iCoverflowDataSource>)_dataSource
{
...
- (void)setDelegate:(id<iCoverflowDelegate>)_delegate
{
...
- (void)setType:(iCoverflowType)_type
{
    if (type != _type)
    {
        type = _type;
...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
	switch (type)
    {
		case iCoverflowTypeTimeMachine:
        case iCoverflowTypeInvertedTimeMachine:
		{
        	if (type == iCoverflowTypeInvertedTimeMachine)
            {
                offset = -offset;
            }
            
#ifdef ICoverflow_MACOS
        ...
        case iCoverflowTypeCustom:
		{
			if ([delegate respondsToSelector:@selector(Coverflow:itemAlphaForOffset:)])
			{
				return [delegate Coverflow:self itemAlphaForOffset:offset];
			}
...
- (CGFloat)valueForTransformOption:(iCoverflowTranformOption)option withDefault:(CGFloat)value
{
    if ([delegate respondsToSelector:@selector(Coverflow:valueForTransformOption:withDefault:)])
    {
        return [delegate Coverflow:self valueForTransformOption:option withDefault:value];
    }
    return value;
}
...
{	
    ...
    switch (type)
    {
		case iCoverflowTypeCustom:
        {
			if ([delegate respondsToSelector:@selector(Coverflow:itemTransformForOffset:baseTransform:)])
			{
				return [delegate Coverflow:self itemTransformForOffset:offset baseTransform:transform];
			}
			
			//deprecated code path
			else if ([delegate respondsToSelector:@selector(Coverflow:transformForItemView:withOffset:)])
			{
				//NSLog(@"Delegate method Coverflow:transformForItemView:withOffset: is deprecated, use Coverflow:transformForItemAtIndex:withOffset:baseTransform: instead.");
				return [delegate Coverflow:self transformForItemView:view withOffset:offset];
			}
			
            //else, fall through to linear transform
        }
        case iCoverflowTypeLinear:
        {
            if (vertical)
            {
        ...
        case iCoverflowTypeRotary:
        case iCoverflowTypeInvertedRotary:
        {
			NSInteger count = [self valueForTransformOption:iCoverflowTranformOptionCount withDefault:
                               MIN(numberOfVisibleItems, numberOfItems + numberOfPlaceholdersToShow)];
            
            CGFloat arc = [self valueForTransformOption:iCoverflowTranformOptionArc withDefault:M_PI * 2.0f];
            CGFloat radius = [self valueForTransformOption:iCoverflowTranformOptionRadius withDefault:fmaxf(itemWidth / 2.0f, itemWidth / 2.0f / tanf(arc/2.0f/count))];
            CGFloat angle = [self valueForTransformOption:iCoverflowTranformOptionAngle withDefault:offset / count * arc];
            
            if (type == iCoverflowTypeInvertedRotary)
            {
                radius = -radius;
                angle = -angle;
            }
        ...
        case iCoverflowTypeCylinder:
        case iCoverflowTypeInvertedCylinder:
        {
			NSInteger count = [self valueForTransformOption:iCoverflowTranformOptionCount withDefault:
                               MIN(numberOfVisibleItems, numberOfItems + numberOfPlaceholdersToShow)];
            
            CGFloat arc = [self valueForTransformOption:iCoverflowTranformOptionArc withDefault:M_PI * 2.0f];
            CGFloat radius = [self valueForTransformOption:iCoverflowTranformOptionRadius withDefault:fmaxf(0.01f, itemWidth / 2.0f / tanf(arc/2.0f/count))];
            CGFloat angle = [self valueForTransformOption:iCoverflowTranformOptionAngle withDefault:offset / count * arc];
            
            if (type == iCoverflowTypeInvertedCylinder)
            {
                radius = -radius;
                angle = -angle;
            }
        ...
        case iCoverflowTypeWheel:
        case iCoverflowTypeInvertedWheel:
        {
            NSInteger count = [self valueForTransformOption:iCoverflowTranformOptionCount withDefault:
                               MIN(numberOfVisibleItems, numberOfItems + numberOfPlaceholdersToShow)];
            
            CGFloat arc = [self valueForTransformOption:iCoverflowTranformOptionArc withDefault:M_PI * 2.0f];
            CGFloat radius = [self valueForTransformOption:iCoverflowTranformOptionRadius withDefault:itemWidth * (CGFloat)count / arc];
            CGFloat angle = [self valueForTransformOption:iCoverflowTranformOptionAngle withDefault:arc / (CGFloat)count];
            
            if (type == iCoverflowTypeInvertedWheel)
            {
                radius = -radius;
                angle = -angle;
            }
        ...
        case iCoverflowTypeCoverFlow:
        case iCoverflowTypeCoverFlow2:
        {
            CGFloat tilt = [self valueForTransformOption:iCoverflowTranformOptionTilt withDefault:0.9f];
            CGFloat spacing = [self valueForTransformOption:iCoverflowTranformOptionSpacing withDefault:0.25f]; // should be ~ 1/scrollSpeed;
            CGFloat clampedOffset = fmaxf(-1.0f, fminf(1.0f, offset));
            
            if (type == iCoverflowTypeCoverFlow2)
            {
                if (toggle >= 0.0f)
                {
                    if (offset <= -0.5f)
        ...
        case iCoverflowTypeInvertedTimeMachine:
		{
			CGFloat tilt = [self valueForTransformOption:iCoverflowTranformOptionTilt withDefault:0.3f];
			CGFloat spacing = [self valueForTransformOption:iCoverflowTranformOptionSpacing withDefault:1.0f];
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //special-case logic for iCoverflowTypeCoverFlow2
    CGFloat clampedOffset = fmaxf(-1.0f, fminf(1.0f, offset));
...
{
	if ([delegate respondsToSelector:@selector(CoverflowItemWidth:)])
    {
		itemWidth = [delegate CoverflowItemWidth:self];
	}
...
- (void)layOutItemViews
{
	...
	//update wrap
	if ([delegate respondsToSelector:@selector(CoverflowShouldWrap:)])
    {
        shouldWrap = [delegate CoverflowShouldWrap:self];
    }
	else
	{
		switch (type)
    ...
    //no placeholders on wrapped Coverflows
    ...
    if ([delegate respondsToSelector:@selector(CoverflowOffsetMultiplier:)])
    {
        offsetMultiplier = [delegate CoverflowOffsetMultiplier:self];
    }
    else
    {
        switch (type)
...
- (UIView *)loadViewAtIndex:(NSInteger)index withContainerView:(UIView *)containerView
{
    ...
    if (index < 0)
    {
		if ([dataSource respondsToSelector:@selector(Coverflow:placeholderViewAtIndex:reusingView:)])
		{
        	view = [dataSource Coverflow:self placeholderViewAtIndex:(int)ceilf((CGFloat)numberOfPlaceholdersToShow/2.0f) + index reusingView:[self dequeuePlaceholderView]];
		}
		
		//deprecated code path
		else if ([dataSource respondsToSelector:@selector(Coverflow:placeholderViewAtIndex:)])
		{
			//NSLog(@"DataSource method Coverflow:placeholderViewAtIndex: is deprecated, use Coverflow:placeholderViewAtIndex:reusingView: instead.");
        	view = [dataSource Coverflow:self placeholderViewAtIndex:(int)ceilf((CGFloat)numberOfPlaceholdersToShow/2.0f) + index];
		}
    }
    else if (index >= numberOfItems)
    {
		if ([dataSource respondsToSelector:@selector(Coverflow:placeholderViewAtIndex:reusingView:)])
		{
        	view = [dataSource Coverflow:self placeholderViewAtIndex:numberOfPlaceholdersToShow/2.0f + index - numberOfItems reusingView:[self dequeuePlaceholderView]];
		}
		...
		{
			//NSLog(@"DataSource method Coverflow:placeholderViewAtIndex: is deprecated, use Coverflow:placeholderViewAtIndex:reusingView: instead.");
        	view = [dataSource Coverflow:self placeholderViewAtIndex:numberOfPlaceholdersToShow/2.0f + index - numberOfItems];
		}
    }
    else if ([dataSource respondsToSelector:@selector(Coverflow:viewForItemAtIndex:reusingView:)])
    {
        view = [dataSource Coverflow:self viewForItemAtIndex:index reusingView:[self dequeueItemView]];
    }
    
	//deprecated code path
	else
    {
		//NSLog(@"DataSource method Coverflow:viewForItemAtIndex: is deprecated, use Coverflow:viewForItemAtIndex:reusingView: instead.");
        view = [dataSource Coverflow:self viewForItemAtIndex:index];
    }
...
{    
    ...
    numberOfItems = [dataSource numberOfItemsInCoverflow:self];
    if ([dataSource respondsToSelector:@selector(numberOfPlaceholdersInCoverflow:)])
    {
        numberOfPlaceholders = [dataSource numberOfPlaceholdersInCoverflow:self];
    }
    
    //get number of visible items
    numberOfVisibleItems = numberOfItems + numberOfPlaceholders;
    if ([dataSource respondsToSelector:@selector(numberOfVisibleItemsInCoverflow:)])
    {
        numberOfVisibleItems = [dataSource numberOfVisibleItemsInCoverflow:self];
    }
...
- (void)scrollByNumberOfItems:(NSInteger)itemCount duration:(NSTimeInterval)duration
{
	if (duration > 0.0)
    {
		...
		if ([delegate respondsToSelector:@selector(CoverflowWillBeginScrollingAnimation:)])
		{
			[delegate CoverflowWillBeginScrollingAnimation:self];
		}
        [self startAnimation];
    }
    else
...
- (void)step
{
    ...
    if (scrolling)
    {
        ...
        if (time == 1.0f)
        {
            scrolling = NO;
            [self depthSortViews];
			if ([delegate respondsToSelector:@selector(CoverflowDidEndScrollingAnimation:)])
			{
				[delegate CoverflowDidEndScrollingAnimation:self];
			}
        }
    }
    else if (decelerating)
    {
        ...
        {
            decelerating = NO;
			if ([delegate respondsToSelector:@selector(CoverflowDidEndDecelerating:)])
			{
				[delegate CoverflowDidEndDecelerating:self];
			}
...
{	
    ...
    if ([delegate respondsToSelector:@selector(CoverflowDidScroll:)])
    {
		[delegate CoverflowDidScroll:self];
	}
    
    //notify delegate of change index
    if ([self clampedIndex:previousItemIndex] != self.currentItemIndex &&
        [delegate respondsToSelector:@selector(CoverflowCurrentItemIndexUpdated:)])
    {
        [delegate CoverflowCurrentItemIndexUpdated:self];
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gesture shouldReceiveTouch:(UITouch *)touch
{
    if ([gesture isKindOfClass:[UITapGestureRecognizer class]])
    {
        ...
        if (index != NSNotFound)
        {
			if ([delegate respondsToSelector:@selector(Coverflow:shouldSelectItemAtIndex:)])
			{
				if (![delegate Coverflow:self shouldSelectItemAtIndex:index])
				{
					return NO;
				}
			}
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    if ([delegate respondsToSelector:@selector(Coverflow:didSelectItemAtIndex:)])
    {
        [delegate Coverflow:self didSelectItemAtIndex:index];
    }
}
    ...
    if (scrollEnabled)
    {
        switch (panGesture.state)
        {
            case UIGestureRecognizerStateBegan:
            {
				...
				if ([delegate respondsToSelector:@selector(CoverflowWillBeginDragging:)])
				{
					[delegate CoverflowWillBeginDragging:self];
				}
                break;
            }
            ...
            case UIGestureRecognizerStateCancelled:
            {
				...
				if ([delegate respondsToSelector:@selector(CoverflowDidEndDragging:willDecelerate:)])
				{
					[delegate CoverflowDidEndDragging:self willDecelerate:decelerating];
				}
				...
				else if ([delegate respondsToSelector:@selector(CoverflowWillBeginDecelerating:)])
				{
					[delegate CoverflowWillBeginDecelerating:self];
				}
				break;
            }
            default:
...
- (void)scrollWheel:(NSEvent *)theEvent
{
	...
	//the iCoverFlow deceleration system conflicts with the built-in momentum
