...
- (void)viewAppearAnimate:(NSString *)value
{
    if ([value isEqualToString:@"appear"])
    {
        ...
        if (self.url) {
            ...
            [weakBanner setImageWithURLRequest:[NSURLRequest requestWithURL:self.url] placeholderImage:[UIImage imageNamed:@""] success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
//                image = [image imageByScalingToSize:CGSizeMake(BANNER_WIDTH, BANNER_IMAGE_ORIGINAL_HEIGHT)];
//                image = [image imageAtRect:CGRectMake(0, 0, BANNER_WIDTH, BANNER_HEIGHT)];
...
//            self.imageView.image = self.hignImage;
            ...
            self.isLoaded = YES;
//             self.imageView.frame = [self getFrameByImageSize:self.imageView.image];
        }
        else
        {
        self.imageView.image = self.hignImage;
