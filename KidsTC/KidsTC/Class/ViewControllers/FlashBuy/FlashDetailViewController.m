...
@interface FlashDetailViewController () <FlashDetailViewDelegate, ServiceDetailBottomViewDelegate, ServiceDetailConfirmViewDelegate, KTCActionViewDelegate, KTCBrowseHistoryViewDataSource, KTCBrowseHistoryViewDelegate>

@property (weak, nonatomic) IBOutlet UIView *detailBGView;
@property (weak, nonatomic) IBOutlet FlashDetailView *detailView;
...
@property (nonatomic, strong) FlashDetailViewModel *viewModel;
...
- (instancetype)initWithServiceId:(NSString *)serviceId channelId:(NSString *)channelId {
    self = [super initWithNibName:@"FlashDetailViewController" bundle:nil];
    if (self) {
...
- (void)viewDidLoad {
    ...
    self.viewModel = [[FlashDetailViewModel alloc] initWithView:self.detailView];
    __weak FlashDetailViewController *weakSelf = self;
...
#pragma mark FlashDetailViewDelegate
...
- (void)browseHistoryView:(KTCBrowseHistoryView *)view didSelectedItemAtIndex:(NSUInteger)index {
    ...
    switch (type) {
        case KTCBrowseHistoryTypeService:
        {
            ...
            FlashDetailViewController *controller = [[FlashDetailViewController alloc] initWithServiceId:model.identifier channelId:model.channelId];
            [controller setHidesBottomBarWhenPushed:YES];
            [self.navigationController pushViewController:controller animated:YES];
            //MTA
