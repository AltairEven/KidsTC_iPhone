...
- (void)image:(UIImage *)image didFinishSavingWithUrl:(NSURL *)url Error:(NSError *)error;

- (void)handleTakenPhotoImage:(UIImage *)image savedUrl:(NSURL *)url;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if([lastChosenMediaType isEqual:(NSString *) kUTTypeImage])
    {
        ...
        if (chosenImage) {
            ...
            [self.assetsLibrary writeImageToSavedPhotosAlbum:[chosenImage CGImage] orientation:(ALAssetOrientation)[chosenImage imageOrientation] completionBlock:^(NSURL *assetURL, NSError *error){
                [self image:chosenImage didFinishSavingWithUrl:assetURL Error:error];
            }];
        }
    } else {
        UIAlertController *controller = [UIAlertController alertControllerWithTitle:@"提示" message:@"系统只支持图片格式" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:nil];
        [controller addAction:action];
        [self presentViewController:controller animated:YES completion:nil];
    }
...
- (void)image:(UIImage *)image didFinishSavingWithUrl:(NSURL *)url Error:(NSError *)error {
    if (error || !url) {
        UIAlertController *controller = [UIAlertController alertControllerWithTitle:@"提示" message:@"拍摄的照片保存失败，将无法查看大图" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *action = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self handleTakenPhotoImage:image savedUrl:url];
        }];
        [controller addAction:action];
        [self presentViewController:controller animated:YES completion:nil];
    } else {
...
- (void)handleTakenPhotoImage:(UIImage *)image savedUrl:(NSURL *)url {
    ...
    CGFloat imageviewWidth = (self.view.frame.size.width - 5*IMAGEVIEW_HORIZONTALGAP)/4;
    CGFloat imageviewHeight = imageviewWidth;
    UIImageView *thumbnailView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, imageviewWidth, imageviewHeight)];
    
    UIImage *photoSelected = [UIImage imageNamed:@"picture_selected"];
    UIImageView *photoSelectedImageview = [[UIImageView alloc]initWithImage:photoSelected];
    [photoSelectedImageview setFrame:CGRectMake(imageviewWidth - 15, 0 , 15, 15)];
    [thumbnailView addSubview:photoSelectedImageview];
    [self.selectedImageViewsArray addObject:thumbnailView];
    
    [self.chosenPhotos addObject:image];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(getTakePicturePhoto:photosDictionary:)]) {
        [self.delegate getTakePicturePhoto:image photosDictionary:dataDic];
    }
    [self.photoPicker dismissViewControllerAnimated:NO completion:nil];
    [self dismiss];
}
