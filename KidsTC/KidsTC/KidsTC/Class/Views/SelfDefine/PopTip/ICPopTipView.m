...
#import "ICPopTipView.h"

@interface ICPopTipView ()
@property (nonatomic, strong, readwrite)	id	targetObject;
@end


@implementation ICPopTipView
...
- (void)drawRect:(CGRect)rect {
	...
	CGContextSetRGBFillColor(c, 0.0, 0.0, 0.0, 0.2);
	CGContextFillPath(c);
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView animated:(BOOL)animated {
	...
	if (animated) {
        if (animation == ICPopTipAnimationSlide) {
            self.alpha = 0.0;
            CGRect startFrame = finalFrame;
            startFrame.origin.y += 10;
            self.frame = startFrame;
        }
		else if (animation == ICPopTipAnimationPop) {
    ...
    {
        NSTimer *timer1 = [NSTimer timerWithTimeInterval:(0.15f+self.hideDuration)
                                                  target:self selector:@selector(autoDismiss:)
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.hideDuration = 0.0;
		self.textFont = [UIFont boldSystemFontOfSize:14.0];
		self.textColor = [UIColor whiteColor];
		self.textAlignment = NSTextAlignmentCenter;
		self.backgroundColor = [UIColor colorWithRed:62.0/255.0 green:60.0/255.0 blue:51.0/255.0 alpha:0.5];
        self.animation = ICPopTipAnimationSlide;
    }
    return self;
}
